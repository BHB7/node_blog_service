...
const sendCodeController = async (req, res) => {
  ...
  if (await getVerifyCode(email)) {
    return res.error(
      "呜呜，验证码已经飞到你的邮箱啦~ 等一下，它会乖乖回来的！"
    );
  }

  if (!email)
    return res.error("哎呀呀，邮箱空空如也哦，快填上一个小小邮箱吧！");
  ...
  try {
    const html = await ejs.renderFile(
      path.join(__dirname, "../", "html", "email.ejs"),
      { code }
    );
    await sendMail({
      to: email,
...
const getUserByIdController = async (req, res) => {
  ...
  try {
    const result = await getUserServiceById(uid);
...
const getAdminController = async (req, res) => {
  try {
    const result = await getUserServiceById();
    res.success(result, "获取成功");
  } catch (err) {
    res.error("获取用户失败");
  }
...
const loginController = async (req, res) => {
  ...
  try {
    ...
    await updateUserInfoService(info.user.id, {
      ip: req.user.ip,
...
const getIpAddressController = async (req, res) => {
  try {
    ...
    console.log(ipInfo);
    res.success(ipInfo.data.data, "获取成功");
  } catch (error) {
    res.error("获取ip信息失败");
  }
};
...
const getUserListController = async (req, res) => {
  try {
    ...
    const pageSize = 10; // 每页条数，也可以从 req.query 中取

    const userData = await getUserAllService(page, pageSize);

    res.success(userData); // 假设你的 res.success 已封装好格式
  } catch (error) {
...
// 冻结用户账户 仅限超级管理员
const setUserFreezeController = async (req, res) => {
  const uid = req.auth?.id || req.user?.id;
  const { tid } =req.query || req.params;
  ...
  // if (userInfo.permissionLevel !== "200") return res.error("权限不足");
  try {
    const flag = await updateUserInfoService(tid, {permissionLevel:String(permissionLevel)});
    if (!flag) return res.error("更新失败");
    res.success(null, "更新成功");
  } catch (error) {
    res.error("更新信息失败");
